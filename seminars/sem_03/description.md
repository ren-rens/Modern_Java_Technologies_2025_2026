...
* SoftwareEngineeringSemesterPlanner където стратегията при записването на предметите ще е да се минимизират броя предмети, с които да се покрият изискванията за семестъра.

* ComputerScienceSemesterPlanner, където стратегията при записването на предметите ще е да се запишат предметите с по-добри отзиви от други състуденти, без да се гледат нужния брой предмети по категорията им.

#### ⚠️ Важно: Интерфейсът не трябва да позволява други имплементации, освен посочените две
...
Както забелязвате, calculateJarCount методът връща броя на бурканите, които ще са нужни за оцеляване. За простота няма да има различна стратегия за имплементацията на този метод.
...
Нужната почивка след всеки предмет е ```времето нужно да се учи за предмета * коефициента на категорията му``` (При нужда нека се закръглява нагоре)
