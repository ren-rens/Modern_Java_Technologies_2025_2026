...
public class Main {
    ...
    public static void main(String... args) throws InvalidSubjectRequirementsException, CryToStudentsDepartmentException, DisappointmentException {
        ...
        ComputerScienceSemesterPlanner planner2 = new ComputerScienceSemesterPlanner();

        UniversitySubject[] subjects2 = {
                new UniversitySubject("Java Programming", 4, 4, Category.PROGRAMMING, 30),
                new UniversitySubject("Linear Algebra", 5, 3, Category.MATH, 35),
                new UniversitySubject("Data Structures", 3, 5, Category.PROGRAMMING,25)
        };

        plan1 = new SemesterPlan(subjects2, requirements, 5);

        selectedSubjects = planner2.calculateSubjectList(plan1);
        printSubjects(selectedSubjects);

        jarCount = planner.calculateJarCount(selectedSubjects, 11, 50);
        System.out.println("Jar count: " + jarCount);
